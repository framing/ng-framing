...
export class LiveExampleComponent implements OnInit {
  ...
  constructor(private elementRef: ElementRef, location: Location ) {
    ...
    function getAttrValue(atty: string | string[]): string {
  ...
  getAttrs(): any {
    ...
    const attrMap = {};
    Object.keys(attrs).forEach((key) => (attrMap as any)[attrs[key].name] = attrs[key].value);
    return attrMap;
  }
  ...
  toggleEmbedded(): void { this.showEmbedded = !this.showEmbedded; }
}

function boolFromAtty(atty: string , def: boolean = false): boolean {
